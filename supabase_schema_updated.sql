-- =============================================================================
-- Gen3FirmwareUpdater Supabase Schema (UPDATED)
-- Run this in the Supabase SQL Editor
-- This version matches the admin tool code
-- =============================================================================
...
CREATE TABLE scooters (
    ...
    model TEXT,
    hw_version TEXT,  -- ADDED: Hardware version field
    notes TEXT,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE firmware_versions (
    ...
    target_hw_version TEXT NOT NULL,  -- Primary target HW version
    min_sw_version TEXT,
    release_notes TEXT,
    access_level TEXT NOT NULL DEFAULT 'distributor',  -- ADDED: 'public' or 'distributor'
    is_active BOOLEAN NOT NULL DEFAULT true,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
-- ---------------------------------------------------------------------------
-- Table: firmware_hw_targets (NEW)
-- Many-to-many relationship: one firmware can target multiple HW versions
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Table: telemetry_snapshots (NEW)
-- Stores telemetry data captured from scooters
-- ---------------------------------------------------------------------------
...
-- Firmware HW targets: read-only for anon
...
-- Telemetry: read, insert for anon
...
INSERT INTO scooters (zyd_serial, distributor_id, model, hw_version, notes)
SELECT 'ZYD1234567890', id, 'Gen3 Pro', 'V1.0', 'Test scooter'
FROM distributors WHERE activation_code = 'TEST-2024-ALPHA';

-- Insert test firmware with proper access_level
INSERT INTO firmware_versions (version_label, file_path, file_size_bytes, target_hw_version, min_sw_version, release_notes, access_level, is_active)
VALUES ('V2.3', 'controller_v2_3.bin', 0, 'V1.0', NULL, 'Test firmware version', 'distributor', true);

-- Insert HW target mapping
INSERT INTO firmware_hw_targets (firmware_version_id, hw_version)
SELECT id, 'V1.0' FROM firmware_versions WHERE version_label = 'V2.3';
